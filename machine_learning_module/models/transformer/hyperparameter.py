"""
TFT-Lite hyperparameter tuner: Optuna search space + grid/random fallbacks,
reusing BaseHyperparameterTuner exactly like the other algorithms. Each
trial fully retrains the network, so keep --n-trials modest.
...
logger = get_logger(__name__, log_file="logs/transformer_tuning.log")

# hidden_size must stay divisible by num_attention_heads -- only combinations
# that satisfy this are offered to the search space below.
_VALID_HIDDEN_HEAD_PAIRS = [(32, 2), (32, 4), (64, 2), (64, 4), (64, 8), (128, 4), (128, 8)]


class TFTTuner(BaseHyperparameterTuner):
    ...
    def search_space(self, trial: Any) -> dict:
        hidden_size, num_heads = trial.suggest_categorical("hidden_head_pair", _VALID_HIDDEN_HEAD_PAIRS)
        return {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune TFT-Lite hyperparameters.")
...
def main() -> None:
    ...
    tuner = TFTTuner(config, TFTLiteModel, bundle)
