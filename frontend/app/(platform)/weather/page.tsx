...
export default async function WeatherPage() {
  const [weather, stations] = await Promise.all([
    getCurrentWeather(),
    getHazardStations(),
  ])
  ...
  return (
    <div className="p-4 lg:p-6 flex flex-col gap-6">
      ...
      <section aria-label="Current conditions" className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard label="Rainfall (1h)" value={weather.rainfall} unit="mm" icon={CloudRain} sub="Monsoon active" tone="warning" />
        <StatCard label="Humidity" value={weather.humidity} unit="%" icon={Droplets} sub={`Dew point ${weather.dewPoint}°C`} />
        <StatCard label="Pressure" value={weather.pressure} unit="hPa" icon={Gauge} sub="Falling trend" tone="warning" />
        <StatCard label="Wind" value={weather.windSpeed} unit="km/h" icon={Wind} sub={`From ${weather.windDirection}`} />
        <StatCard label="Visibility" value={weather.visibility} unit="km" icon={Eye} sub="Reduced in valleys" />
        <StatCard label="UV Index" value={weather.uvIndex} icon={Sun} sub="Moderate" />
        <StatCard label="Cloud Cover" value={weather.cloudCover} unit="%" icon={Cloud} sub="Convective build-up" tone="warning" />
      </section>
