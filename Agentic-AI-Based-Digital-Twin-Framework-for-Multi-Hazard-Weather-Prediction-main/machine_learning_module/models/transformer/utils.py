"""Transformer (TFT-Lite) helper functions (CLI arg parsing / config building)."""
...
def build_config_from_args(args: argparse.Namespace) -> TFTLiteConfig:
    config = TFTLiteConfig(
...
def add_common_cli_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--experiment-name", dest="experiment_name", default="tft_experiment")
...
def add_tft_cli_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--group-column", dest="group_column", default=None,
                         help="Column name (must be present in X_*.csv) identifying separate time series "
                              "(e.g. a district/station id). Handled automatically -- do not also pass it via --drop-columns.")
    parser.add_argument("--hidden-size", dest="hidden_size", type=int, default=64)
    parser.add_argument("--num-attention-heads", dest="num_attention_heads", type=int, default=4,
                         help="Must evenly divide --hidden-size.")
    parser.add_argument("--num-lstm-layers", dest="num_lstm_layers", type=int, default=1)
