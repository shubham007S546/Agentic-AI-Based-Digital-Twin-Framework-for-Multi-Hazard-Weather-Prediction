...
from app.core.enums import District, UserRole
...
async def get_all_reports(
    ...
    owner_id = None if token_data.role in (UserRole.ADMIN.value, UserRole.SUPER_ADMIN.value) else uuid.UUID(token_data.user_id)
    reports = await controller.get_all_reports(limit, offset, district, owner_id)
...
async def get_report(
    ...
    owner_id = None if token_data.role in (UserRole.ADMIN.value, UserRole.SUPER_ADMIN.value) else uuid.UUID(token_data.user_id)
    report = await controller.get_report(report_id, owner_id)
