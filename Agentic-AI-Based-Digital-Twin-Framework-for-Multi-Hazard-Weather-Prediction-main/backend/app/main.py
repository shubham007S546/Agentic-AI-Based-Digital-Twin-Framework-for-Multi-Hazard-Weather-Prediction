...
def create_application() -> FastAPI:
    ...
    # The Prometheus FastAPI instrumentator is incompatible with the app's nested
    # router composition in local dev/test runs, so we only enable it in production
    # where the metrics endpoint is required and the runtime is intentionally fixed.
    ...
    if settings.otel.enabled and settings.app.env == "production":
